...
static void current() 
{
   ...
   if(current_tm.tm_min==60){
      current_tm.tm_min=0;
      current_tm.tm_hour++;
   }
   if(current_tm.tm_hour==24){
   ...
   if(current_tm.tm_mday==date(current_tm.tm_year, current_tm.tm_mon)+1){
...
void day_set()
{
      ...
      if (btn == 'b') {
         ...
         if (current_tm.tm_mday == date(current_tm.tm_year, current_tm.tm_mon)+1) set_min(st_info);
         else plus_one(st_info);
      }
      if (btn == 'c') {
         st_info = SEC_SET;
...
void plus_one(int st_info)
{
   switch (st_info)
   {
      ...
      case YEAR_SET:    
         for(int i=0; i<12; i++) {
            current_tm.tm_wday = (current_tm.tm_wday += (date(current_tm.tm_year, (current_tm.tm_mon+i)%12)))%7;
         }
         current_tm.tm_year++;
         break;
      case MONTH_SET: 
         current_tm.tm_wday = (current_tm.tm_wday += date(current_tm.tm_year, current_tm.tm_mon))%7;
         current_tm.tm_mon++;
         break;
      case DAY_SET:
         current_tm.tm_mday++;
...
void set_min(int st_info)
{
   switch (st_info)
   {
      ...
      case YEAR_SET:
         ...
         current_tm.tm_wday = (current_tm.tm_wday += 5)%7;
         break;
      case MONTH_SET:
         current_tm.tm_mon = 1;
         current_tm.tm_wday = (current_tm.tm_wday += 2)%7;
         break;
      case DAY_SET:
         current_tm.tm_mday = 1;
         current_tm.tm_wday = (current_tm.tm_wday += date(current_tm.tm_year, current_tm.tm_mon)%7+2)%7;
         break;
      case AL_HOUR_SET:
         alarm_tm.tm_hour = 0;
         break;
